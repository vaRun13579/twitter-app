...
app.get('/user/tweets', authentication, async (request, response) => {
  let {username, user_id} = request
  let query1 = `select t1.tweet, t1.likes, t2.replies, t1.date_time as dateTime from (select tweet.tweet, count(like.like_id) as likes, tweet.date_time from tweet left join like on tweet.tweet_id=like.tweet_id where tweet.user_id=${user_id} group by tweet, date_time) as t1 join (select tweet.tweet, count(reply.reply) as replies
